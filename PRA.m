function [prx,cyclenum,pr,pr_sig,pr_all] = PRA(dat,orbit,dt,sr1,sr2,srstep,red,nsim,pad,smoothwin,linlog,detrended,plotn,display,method_powerspec,win,method_smooth,method_cutoff,cutoff,parallelcomputing,cutoff_value) 
...
[prx,nmi,pr,pr_all] = cyclepr(data,orbit,sr1,sr2,srstep,method_cutoff,cutoff,cutoff_value);
mpts = length(prx);
orbitn = length(orbit);
cyclenum = (orbitn-nmi);
...
if parallelcomputing == 0
    ...
    for i = 1: nsim
        ...
        [pr_rand,~] = cycleprsig(sim_spectum,orbit,sr1,sr2,srstep,method_cutoff,cutoff,cutoff_value);
        if display == 1
